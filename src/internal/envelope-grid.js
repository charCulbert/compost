...
export function visibleEnvelopeGridStep(grid, duration, width, minimumPixels = 12) {
...
export function envelopeValueGuides(min, max, {
  height = 1, scale = 'linear', stepped = false, step = 0, minimumPixels = 12,
} = {}) {
  ...
  const selected = [];
  for (const value of [...new Set(values)].sort((a, b) => envelopeValueToY(a, range, rowHeight, scale)
    ...
    if (zeroGuide && value === 0) {
      for (let index = selected.length - 1; index >= 0; index -= 1) {
        if (Math.abs(envelopeValueToY(selected[index], range, rowHeight, scale) - y) < minimum) {
          selected.splice(index, 1);
        }
      }
      selected.push(value);
    } else if (selected.every((entry) => Math.abs(
      envelopeValueToY(entry, range, rowHeight, scale) - y) >= minimum)) {
      selected.push(value);
    }
  }
