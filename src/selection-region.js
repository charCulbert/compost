...
export function normalizeSelectionRegion(start, end, items, max = Number.POSITIVE_INFINITY) {
  const first = Number(start);
  const last = Number(end);
  if (!Number.isFinite(first) || !Number.isFinite(last)) return null;
  const limit = Number.isFinite(Number(max)) ? Math.max(0, Number(max)) : Number.POSITIVE_INFINITY;
...
export function extendSelectionRegion(region, beat, anchor, items, max = Number.POSITIVE_INFINITY) {
  return normalizeSelectionRegion(region?.start ?? anchor, beat, items ?? region?.items, max);
}
