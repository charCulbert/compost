...
export class CompostNoteEditor extends HTMLElement {
	...
	constructor() {
		...
		/** @type {{beat: number, note: number, x: number, y: number, time: number}|null} */ this.pendingEmptyClick =
			null;
      ...
      <div class="tip" part="tip" hidden></div>`;
		...
		this.gridElement.addEventListener("dblclick", (event) =>
		...
		this.keys.addEventListener("pointerdown", (event) =>
			this.startKeyPan(event),
		);
		this.keys.addEventListener("pointermove", (event) =>
			this.moveKeyPan(event),
		);
		this.keys.addEventListener("pointerup", (event) => this.endKeyPan(event));
		this.keys.addEventListener("pointercancel", (event) =>
			this.endKeyPan(event),
		);
		...
		this.keys.addEventListener(
			"wheel",
			(event) => this.handleKeysWheel(event),
			{ passive: false },
		);
		for (const [node, kind] of [
			...
			node.addEventListener("pointerdown", (event) =>
				this.startMarkerDrag(event, "loop", kind),
			);
		...
		for (const [node, kind] of [
			...
			[this.rangeEndHandle, "end"],
		]) {
			node.addEventListener("pointerdown", (event) =>
				this.startMarkerDrag(event, "range", kind),
			);
			node.addEventListener("pointermove", (event) =>
				this.moveMarkerDrag(event),
			);
	...
	syncAttributes() {
		...
		const rawEnd = Math.max(
			rawStart + MIN_DURATION,
			numberAttr(this, "end", this.rangeEnd),
		);
		const rawLoopStart = Math.max(0, numberAttr(this, "loop-start", rawStart));
		const rawLoopEnd = Math.max(
			rawLoopStart + MIN_DURATION,
			numberAttr(this, "loop-end", rawEnd),
		);
		...
		this.scaleRoot = this.hasAttribute("root")
			? ((Math.round(numberAttr(this, "root", 0)) % 12) + 12) % 12
			: null;
		...
		this.beats = this.explicitBeats
			? Math.max(
					this.rangeEnd,
					this.loopEnd,
					numberAttr(this, "beats", this.beats),
				)
			: Math.max(Math.max(this.rangeEnd, this.loopEnd) + 8, 16);
		this.rootNote = clamp(
			Math.round(numberAttr(this, "root-note", this.rootNote)),
			0,
			127,
		);
		this.noteCount = clamp(
			Math.round(numberAttr(this, "note-count", this.noteCount)),
			MIN_ROWS,
			128,
		);
		this.beatWidth = Math.max(0, numberAttr(this, "beat-width", 0));
		this.playhead = this.hasAttribute("playhead")
			? numberAttr(this, "playhead", 0)
			: null;
		this.defaultVelocity = clamp(
			Math.round(numberAttr(this, "velocity", this.defaultVelocity)),
			1,
			127,
		);
		this.defaultChannel = clamp(
			Math.round(numberAttr(this, "channel", this.defaultChannel)),
			0,
			15,
		);
	}
	...
	currentGridText() {
		return this.adaptiveGrid
			? gridTextForStep(this.step, this.beatsPerBar)
			: gridText(this.grid, this.beatsPerBar);
	}
	...
	newNoteId() {
		...
		if (typeof id !== "string" || !id) {
			throw new Error(
				"compost-note-editor needs a noteIdFactory to create notes",
			);
		}
		return id;
	}
	...
	duplicateSelection() {
		...
		if (this.selectionRegion) {
			const shift = Math.max(
				...copies.map((copy, index) => copy.start - originals[index].start),
			);
			this.selectionRegion = {
				...this.selectionRegion,
	...
	addNote() {
		...
		const start = clamp(
			this.snapBeat(raw),
			0,
			Math.max(0, this.beats - this.step),
		);
		const created = {
			id: this.newNoteId(),
			note:
				this.visibleKeys[Math.floor(this.visibleKeys.length / 2)] ??
				this.rootNote,
			start,
	...
	loopToSelection() {
		...
		this.setRange(
			...
			Math.max(this.rangeEnd, span.end),
			false,
		);
	...
	/** @param {number} x */
	xToBeat(x) {
		return x / this.pxPerBeat;
	}

	get rowHeight() {
		return (
			(this.gridWrap?.clientHeight || 300) /
			Math.max(1, this.visibleKeys.length)
		);
	}
	...
	computeVisibleKeys() {
		if (this.hasAttribute("fold")) {
			const used = [...new Set(this._notes.map((note) => note.note))].sort(
				(a, b) => b - a,
			);
	...
	startPinch() {
		...
		const pitchPosition = clamp(
			(centerY - rect.top) / Math.max(1, rect.height),
			0,
			1,
		);
		this.pinch = {
	...
	movePinch() {
		...
		const noteCount = clamp(
			Math.round((this.pinch.noteCount * this.pinch.yDistance) / yDistance),
			MIN_ROWS,
			MAX_ROWS,
		);
		const rect = this.gridWrap.getBoundingClientRect();
		...
		const rootNote = clamp(
			Math.round(this.pinch.pitch - noteCount * (1 - pitchPosition)),
			0,
			128 - noteCount,
		);
		if (noteCount !== this.noteCount)
			this.setAttribute("note-count", String(noteCount));
		if (rootNote !== this.rootNote)
			this.setAttribute("root-note", String(rootNote));
		this.zoomPxPerBeat = pxPerBeat;
		this.offset = clamp(
			this.pinch.beat * pxPerBeat - (centerX - rect.left),
			0,
			Math.max(0, this.beats * pxPerBeat - width),
		);
		this.refresh();
	}
	...
	renderSelectionRegion() {
		...
		if (box) {
			const rows = this.visibleKeys
				...
				.filter(
					({ note }) =>
						note >= Math.min(...this.selectionRegion.pitches) &&
	...
	renderKeys() {
		...
		this.visibleKeys.forEach((note, index) => {
			...
			const label =
				height >= 9 && (fold || note % 12 === 0) ? " data-label" : "";
			const relative =
				this.scaleRoot === null
					? -1
					: (((note - this.scaleRoot) % 12) + 12) % 12;
			...
			markup.push(
				`<div class="${classes}" part="${parts}" data-note="${note}" data-name="${noteName(note)}"${label}${scaleState}${rootState} style="top:${(index * height).toFixed(2)}px;height:${Math.max(2, height).toFixed(2)}px"></div>`,
			);
		});
		this.keys.innerHTML = markup.join("");
		if (this.hoverNote !== null) {
			this.keys
				.querySelector(`[data-note="${this.hoverNote}"]`)
	...
	renderGrid() {
		for (const node of this.gridElement.querySelectorAll(".gl,.rl,.rw"))
			node.remove();
		const height = this.rowHeight;
		const px = this.pxPerBeat;
		const markup = [];
		this.visibleKeys.forEach((note, index) => {
			...
			if (!isNaturalNote(note))
				markup.push(
					`<div class="rw" part="row" style="top:${top.toFixed(2)}px;height:${height.toFixed(2)}px"></div>`,
				);
			if (note % 12 === 0)
				markup.push(
					`<div class="rl octave" part="row-line octave-line" style="top:${top.toFixed(2)}px"></div>`,
				);
		});
	...
	renderNotes() {
		...
		for (const note of this._preview ?? this._notes) {
			...
			if (note.start < this.rangeStart || note.start >= this.rangeEnd)
				element.dataset.out = "";
			...
			element.style.setProperty(
				"--note-fill",
				`color-mix(in srgb, var(--compost-note-editor-signal) ${fill}%, transparent)`,
			);
			...
			const name =
				height >= 9
					? `<span class="nn" part="note-label">${noteName(note.note)}</span>`
					: "";
	...
	renderSelection() {
		for (const element of this.gridElement.querySelectorAll(".note")) {
			element.toggleAttribute(
				"data-selected",
				this.selection.has(element.dataset.id),
			);
		}
	}
	...
	preview(note, duration = 0) {
		...
		if (duration > 0)
			this.previewTimer = setTimeout(() => this.endPreview(note), duration);
	}
	...
	endPreview(note = null) {
		if (
			!this.activePreview ||
			(note !== null && this.activePreview.note !== note)
		)
			return;
		...
		this.activePreview = null;
		this.dispatchEvent(
			new CustomEvent("note-preview-end", {
				bubbles: true,
				composed: true,
				detail,
			}),
	...
	startKeyPan(event) {
		...
		const key = event
			.composedPath()
			.find(
				(node) => node instanceof HTMLElement && node.classList.contains("key"),
			);
		...
		this.keyPan = {
			...
			root: this.rootNote,
			rows: this.noteCount,
			middle: this.rootNote + this.noteCount / 2,
			note,
	...
	moveKeyPan(event) {
		...
		if (pan.axis === "zoom") {
			...
			this.setAttribute(
				"root-note",
				String(clamp(Math.round(pan.middle - rows / 2), 0, 128 - rows)),
			);
		} else {
			const root = clamp(
				pan.root + Math.round(dy / this.rowHeight),
				0,
				128 - this.noteCount,
			);
	...
	startSelection(event, kind, point = this.gridPoint(event)) {
		...
		this.drag = {
			...
			y: event.clientY,
			startBeat: clamp(
				this.snapBeat(this.xToBeat(point.x), this.gestureIsFree(event)),
				0,
				this.beats,
			),
			createBeat: clamp(
				this.creationBeat(this.xToBeat(point.x), this.gestureIsFree(event)),
				0,
				this.beats,
			),
			...
			regionBefore: this.selectionRegion
				? {
						...this.selectionRegion,
						pitches: this.selectionRegion.pitches
							? [...this.selectionRegion.pitches]
							: undefined,
					}
				: null,
	...
	startRulerSelection(event) {
		if (this.readonly || event.button !== 0 || this.drag || this.loopDrag)
			return;
		if (
			event
				.composedPath()
	...
	setCopyDrag(drag, copying) {
		...
		if (copying) {
			const sources = this._notes.filter((note) =>
				drag.sourceIds.includes(note.id),
			);
			if (!drag.copies)
				drag.copies = sources.map((note) => ({
					...note,
					id: this.newNoteId(),
				}));
			...
			drag.note =
				drag.copies[
					sources.findIndex((note) => note.id === drag.sourceNoteId)
				] ?? drag.copies[0];
	...
	dragSource(drag) {
		return drag.copy
			? normaliseNotes([...this._notes, ...(drag.copies ?? [])], this.beats)
			: this._notes;
	}
	...
	applyMoveDrag(drag, clientX, clientY, free) {
		...
		if (!origin) return;
		const target = snapWithOffset(
			origin.start + deltaBeats,
			origin.start,
			this.step,
			free ? "off" : "grid",
			this.musicalOriginBeat,
		...
		const shiftNote =
			originRow >= 0
				? (this.visibleKeys[targetRow] ?? origin.note) - origin.note
				: 0;
		this._preview = resolveOverlaps(
			movedNotes(
				source,
				drag.ids,
				shiftBeats,
	...
	startPointer(event) {
		...
		const regionBefore = this.selectionRegion
			? {
					...this.selectionRegion,
					pitches: this.selectionRegion.pitches
						? [...this.selectionRegion.pitches]
		...
		if (!element) {
			...
			if (this.hasAttribute("draw")) {
				const start = this.creationBeat(
					this.xToBeat(point.x),
					this.gestureIsFree(event),
				);
				const selectionBefore = [...this.selection];
				const created = {
					id: this.newNoteId(),
					note: this.yToNote(point.y),
				...
				this._preview = resolveOverlaps(
					normaliseNotes([...this._notes, created], this.beats),
					[created.id],
				);
				this.selection = new Set([created.id]);
				this.drag = {
	...
	movePointer(event) {
		...
		const point = this.gridPoint(event);
		if (
			Math.hypot(event.clientX - drag.x, event.clientY - drag.y) > DRAG_SLOP
		) {
		...
		if (drag.mode === "marq") {
			if (
				!drag.moved &&
				Math.hypot(event.clientX - drag.x, event.clientY - drag.y) <= drag.slop
			)
				return;
			if (!drag.moved) {
				drag.moved = true;
			...
			const last = clamp(
				this.snapBeat(this.xToBeat(point.x), free),
				0,
				this.beats,
			);
			...
			const height =
				drag.kind === "box"
					? Math.abs(currentY - drag.y0)
					: this.gridWrap.clientHeight;
			...
			for (const note of notesInBox(this._notes, box)) {
				...
				if (top >= 0 && top + rowHeight > y && top < y + height)
		...
		if (drag.mode === "vel") {
			...
			const source = drag.created
				? normaliseNotes([...this._notes, drag.note], this.beats)
				: this._notes;
			const origin = source.find(
				(/** @type {RollNote} */ entry) => entry.id === drag.note.id,
			);
			if (!origin) return;
			const delta = (this.xToBeat(point.x) - drag.grabBeat) * factor;
			const duration = snapWithOffset(
				origin.duration + delta,
				origin.duration,
				this.step,
				free ? "off" : "grid",
			);
			// every selected note takes the same change of length as the one being dragged
			this._preview = resizedNotes(
				source,
				drag.ids,
			...
			if (drag.created) {
				this._preview = velocityShiftedNotes(
					this._preview,
					drag.ids,
					(drag.y - event.clientY) * factor,
				);
			}
			const current = this._preview.find((entry) => entry.id === drag.note.id);
			if (current && drag.created)
				this.showTip(`vel ${current.velocity}`, event);
		} else if (drag.mode === "lenL") {
			const origin = this._notes.find(
				(/** @type {RollNote} */ entry) => entry.id === drag.note.id,
			);
			if (!origin) return;
			const delta = (this.xToBeat(point.x) - drag.grabBeat) * factor;
			const start = snapWithOffset(
				origin.start + delta,
				origin.start,
				this.step,
				free ? "off" : "grid",
	...
	endPointer(event) {
			...
			if (drag.moved) {
				...
				if (this.selectionRegion) {
					const box = {
						...
						toBeat: this.selectionRegion.end,
				...
				if (drag.kind === "box") {
					...
					const doubleClick =
						pending &&
						time - pending.time <= DOUBLE_CLICK_MS &&
		...
		if (drag.mode === "vel" && !changed) {
			...
			if (
				this.modifiedClick?.id === drag.note.id &&
				now - this.modifiedClick.time <= DOUBLE_CLICK_MS
			) {
				this.modifiedClick = null;
				this.ignoreDoubleClick = true;
				setTimeout(() => {
		...
		if (changed || drag.created || drag.copy) {
			const geometry =
				drag.mode === "move" || drag.mode === "len" || drag.mode === "lenL";
			this.commit(
				this._preview ?? this._notes,
				geometry || drag.created || drag.copy ? drag.ids : null,
			);
	...
	handleDoubleClick(event) {
		...
		if (noteElement) {
			if (event.metaKey || event.ctrlKey) {
				...
				this.commit(
					this._notes.map((note) =>
						note.id === noteElement.dataset.id
							? { ...note, velocity: this.defaultVelocity }
							: note,
					),
				);
	...
	startMarkerDrag(event, scope, kind) {
		...
		/** @type {HTMLElement} */ (event.currentTarget).setAttribute(
			"data-on",
			"",
		);
		/** @type {HTMLElement} */ (event.currentTarget).setPointerCapture(
			event.pointerId,
		);
	}
	...
	moveMarkerDrag(event) {
		...
		const factor = this.gestureFactor(event);
		const step = this.step;
		const quantise = (/** @type {number} */ value) =>
			free ? Math.max(0, value) : Math.round(value / step) * step;
		const deltaBeats = ((event.clientX - drag.x) / drag.px) * factor;
		let start = drag.start;
		let end = drag.end;
		const minimum = free ? MIN_DURATION : step;
		if (drag.kind === "end")
			end = Math.max(start + minimum, quantise(drag.end + deltaBeats));
		else if (drag.kind === "start")
			start = clamp(quantise(drag.start + deltaBeats), 0, end - minimum);
		else {
	...
	handleKey(event) {
			...
			const element =
				id == null
					...
					: this.gridElement.querySelector(
							`.note[data-id="${CSS.escape(String(id))}"]`,
						);
		...
		if (event.key === "Delete" || event.key === "Backspace") {
			...
			if (this.hasAttribute("fold"))
				this.moveSelectionThroughVisiblePitches(direction);
			else {
	...
	handleWheel(event) {
		...
		if (event.altKey) {
			this.setAttribute(
				...
				String(
					clamp(
						Math.round(this.noteCount + (delta > 0 ? 2 : -2)),
						MIN_ROWS,
						MAX_ROWS,
					),
				),
		...
		if (event.metaKey || event.ctrlKey) {
			...
			this.offset = clamp(
				this.offset + (event.deltaX || delta),
				0,
				Math.max(0, this.beats * this.pxPerBeat - width),
			);
		} else {
			const lo = clamp(
				this.rootNote + (delta > 0 ? -1 : 1),
				0,
				128 - this.noteCount,
			);
	...
	handleKeysWheel(event) {
		...
		const middle = this.rootNote + this.noteCount / 2;
		const rows = clamp(
			Math.round(this.noteCount + (event.deltaY > 0 ? 2 : -2)),
			MIN_ROWS,
			MAX_ROWS,
		);
		this.setAttribute("note-count", String(rows));
		this.setAttribute(
			"root-note",
			String(clamp(Math.round(middle - rows / 2), 0, 128 - rows)),
		);
	}
}
