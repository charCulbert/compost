...
export class CompostDeviceSelector extends HTMLElement {
	...
	applySnapshot(snapshot, { requestId = null } = {}) {
		if (
			requestId !== null &&
			requestId !== undefined &&
			Number(requestId) < this.requestId
		) {
			return false;
		}
	...
	handleRefreshClick() {
		...
		this.dispatchEvent(
			new CustomEvent("device-settings-refresh", {
				bubbles: true,
				composed: true,
				detail: {
					requestId,
	...
	handleSettingChange(event) {
		...
		this.dispatchEvent(
			new CustomEvent("device-settings-input", {
				bubbles: true,
				composed: true,
				detail,
			}),
	...
	refresh() {
		...
		this.openButton.setAttribute(
			"aria-expanded",
			this.dialog.open ? "true" : "false",
		);
		this.heading.textContent = heading;
		this.panel.setAttribute("aria-busy", this.busy ? "true" : "false");

		this.fillStringSelect(
			this.selects.audioSystem,
			audio.apis,
			audio.api,
			"Default",
		);
		...
		this.fillNumberSelect(
			this.selects.sampleRate,
			audio.sampleRates,
			audio.sampleRate,
			formatSampleRate,
		);
		this.fillNumberSelect(
			this.selects.bufferSize,
			audio.bufferSizes,
			audio.bufferSize,
			formatBufferSize,
		);
		this.renderMIDICheckboxes(
			this.midiInputs,
			midi.inputDevices,
			midi.inputDeviceIds,
			{
		...
		this.fields.sampleRate.hidden =
			!audio.sampleRate && audio.sampleRates.length === 0;
		this.fields.bufferSize.hidden =
			!audio.bufferSize && audio.bufferSizes.length === 0;
	...
	renderMIDICheckboxes(
		container,
		devices,
		selectedIds,
		{ setting, emptyLabel, idPrefix },
	) {
