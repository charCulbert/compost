...
export class CompostPiano extends HTMLElement {
	...
	constructor({
		...
		this.root.addEventListener("mousedown", (event) =>
			this.handleMouse(event, true, false),
		);
		this.root.addEventListener("mouseup", (event) =>
			this.handleMouse(event, false, true),
		);
		this.root.addEventListener("mousemove", (event) =>
			this.handleMouse(event, false, false),
		);
	...
	get config() {
		const rootNote = Math.max(
			0,
			Math.min(127, numberAttr(this, "root-note", 36)),
		);
	...
	isNoteActive(note) {
		return (
			note === this.draggedNote ||
			this.keyboardNotes.has(note) ||
			this.externalNotes.has(note)
		);
	}
	...
	handleTouchMove(event) {
		for (const touch of event.changedTouches) {
			...
			const element = this.root.elementFromPoint?.(
				touch.clientX,
				touch.clientY,
			);
	...
	refreshHTML() {
		...
		for (const child of this.root.children) {
			...
			child.addEventListener("touchcancel", (event) =>
	...
	refreshActiveNotes() {
		for (const { note, element } of this.notes) {
			if (element) {
				...
				const keyPart = element.classList.contains("natural-note")
					? "key natural-key"
					: "key accidental-key";
				element.classList.toggle("active", active);
				element.setAttribute(
					"part",
					active ? `${keyPart} active-key` : keyPart,
				);
			}
		}
	}
