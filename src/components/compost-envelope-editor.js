...
} from "../internal/envelope-grid.js";
import {
	createLongPress,
	DOUBLE_TAP_DISTANCE,
	DOUBLE_TAP_MS,
...
export class CompostEnvelopeEditor extends HTMLElement {
	...
	syncAttributes() {
		...
		const range = envelopeRange(
			numberAttr(this, "min", this.min),
			numberAttr(this, "max", this.max),
		);
		this.min = range.min;
		this.max = range.max;
		this.scale = this.getAttribute("scale") === "gain" ? "gain" : "linear";
		this.stepped = this.hasAttribute("stepped");
		this.step = effectiveEnvelopeStep(
			this.stepped,
			this.hasAttribute("step") ? this.getAttribute("step") : undefined,
		);
		const grid = Number(this.getAttribute("grid"));
		this.grid =
			this.hasAttribute("grid") && grid > 0 && Number.isFinite(grid)
				? grid
				: null;
		this.snapMode =
			this.grid && this.getAttribute("snap") !== "off" ? "grid" : "off";
	...
	set points(points) {
		...
		this.selectionPointIndexes = this.selectionPointIndexes.filter(
			(index) => index >= 0 && index < this._points.length,
		);
		this.render();
	}
	...
	set gridLines(value) {
		if (value === "off" || value === "time")
			this.setAttribute("grid-lines", value);
	...
	setSelection(start, end) {
		...
		this.selectionPointIndexes = this.selection
			? this._points.flatMap((point, index) =>
					point.time >= low && point.time <= high ? [index] : [],
				)
			: [];
		this.render();
	}
	...
	size() {
		return {
			width: Math.max(1, this.surface.clientWidth || this.clientWidth || 1),
			height: Math.max(1, this.surface.clientHeight || this.clientHeight || 1),
		};
	}

	x(time, width = this.size().width) {
		return clamp(Number(time) / this.duration, 0, 1) * width;
	}
	y(value, height = this.size().height) {
		return envelopeValueToY(value, this.min, this.max, height, this.scale);
	}
	...
	paintGrid(width, height) {
		...
		const visibleStep = visibleEnvelopeGridStep(
			this.grid,
			this.duration,
			width,
		);
		if (visibleStep) {
			this.gridElement.style.backgroundImage =
				"linear-gradient(to right, var(--compost-envelope-grid) 1px, transparent 1px)";
	...
	render(points = this._points) {
		...
		this.selectionHighlight.setAttribute(
			"d",
		...
		points.forEach((point, index) => {
			...
			const x = this.x(point.time, width);
			const y = this.y(point.value, height);
			...
			marker.setAttribute(
				"aria-label",
				`${this.label} point ${Number(point.time).toFixed(2)} ${Number(point.value).toFixed(2)}`,
			);
	...
	selectionPath(points, width, height) {
		...
		const selected = sliceEnvelopeRange(
			points,
	...
	pointFromEvent(event) {
		const marker = event
			.composedPath()
			.find(
				(node) => node instanceof Element && node.classList.contains("point"),
			);
		return marker instanceof Element
			? { marker, index: Number(marker.dataset.pointIndex) }
			: null;
	}
	...
	timeAtPointer(event, free = false) {
		...
		const raw = clamp(
			((event.clientX - rect.left) / Math.max(1, rect.width)) * this.duration,
			0,
			this.duration,
		);
	...
	segmentIndex(time) {
		...
		for (let index = 0; index < this._points.length - 1; index += 1) {
			if (
				time >= this._points[index].time &&
				time <= this._points[index + 1].time
			)
				return index;
		}
	...
	curveTargetAtPointer(event) {
		...
		if (
			...
			Math.abs(distance) <= TOUCH_SEGMENT_DISTANCE &&
			rawTime >= first &&
			rawTime <= last
		) {
		...
		if (
			Math.abs(distance) > POINT_PREVIEW_DISTANCE &&
			Math.abs(distance) <= SEGMENT_HANDLE_DISTANCE &&
			rawTime >= first &&
			rawTime <= last
		) {
	...
	startPointer(event) {
		...
		const selectedPoint =
			point && this.selectionPointIndexes.includes(point.index);
		const mode = this.draw
			? "draw"
			: selectedPoint
				? "range"
				: point
					? "point"
					: curveTarget?.kind === "segment"
						? "segment"
						: curveTarget?.kind === "point"
							? "insert"
							: "selection";
		this.drag = {
			pointerId: event.pointerId,
		...
		this.segmentHighlight.setAttribute(
			"d",
			mode === "segment"
	...
	movePointer(event) {
		...
		if (!drag) {
			...
			const value = point
				? this._points[point.index]?.value
				: envelopeValueAtTime(
						this._points,
						time,
						this.min,
		...
		if (drag.curvePointerId === event.pointerId) {
			...
			const curve = clamp(
				(Number(before.curve) || 0) +
					((event.clientY - drag.curveStartY) / height) * 2 * direction,
				-1,
				1,
			);
			const points = origin.map((point, index) =>
				index === drag.segmentIndex
			...
			this.segmentHighlight.setAttribute(
				"d",
				this.segmentPath(drag.segmentIndex, points),
			);
		...
		const rect = this.surface.getBoundingClientRect();
		const factor = event.shiftKey ? 0.25 : 1;
		...
		if (drag.mode === "point") {
			...
			points = moveEnvelopePoint(
				drag.origin,
				drag.pointIndex,
				{
					time: origin.time + (time - origin.time) * factor,
					value: moveEnvelopePointsByY([origin], [0], dy * factor, options)[0]
						.value,
				},
				this.min,
				this.max,
			...
			if (event.altKey && !this.stepped) {
				...
				const curve = clamp(
					(Number(before.curve) || 0) +
						(dy / Math.max(1, rect.height)) * 2 * direction,
					-1,
					1,
				);
				points = drag.origin.map((point, index) =>
					index === drag.segmentIndex
						? { ...point, curve: Math.abs(curve) < 1e-9 ? 0 : curve }
						: { ...point },
				);
				readoutTime = (before.time + after.time) / 2;
				readoutValue = envelopeValueAtTime(
					points,
					readoutTime,
					this.min,
					this.max,
				...
				points = moveEnvelopePointsByY(
					drag.origin,
					[drag.segmentIndex, drag.segmentIndex + 1],
					dy * factor,
					options,
				);
			}
			...
			const deltaTime = clamp(
				(time - drag.startTime) * factor,
				-selection.start,
				this.duration - selection.end,
			);
			const tagged = moveEnvelopePointsByY(
				drag.origin,
				indexes,
				dy * factor,
				options,
				...
				.map((point, index) => ({
					point: selected.has(index)
						? { ...point, time: point.time + deltaTime }
						: point,
					selected: selected.has(index),
				}))
				.sort((a, b) => a.point.time - b.point.time);
			points = tagged.map(({ point }) => point);
			drag.previewSelectionPointIndexes = tagged.flatMap(
				({ selected: isSelected }, index) => (isSelected ? [index] : []),
			);
	...
	endPointer(event) {
		...
		if (drag.mode === "range" && drag.previewSelection)
			this.dispatchSelection();
		this.render();
		this.dispatchEvent(
			eventOf("envelope-change", {
	...
	addAtPointer(
		event,
		pointIndex = this.pointFromEvent(event)?.index ?? -1,
		...
		event.stopPropagation();
		const added = {
			time: this.timeAtPointer(event, this.freeTime(event)),
		...
		if (pointIndex < 0)
			for (let index = 0; index < points.length; ++index)
				if (
					points[index].time === added.time &&
					points[index].value === added.value
				)
					addedIndex = index;
	...
	handleKey(event) {
		...
		if (event.key === "Delete" || event.key === "Backspace") {
			...
			this.dispatchEvent(
				eventOf("envelope-change", {
					points: deleteEnvelopePoint(this._points, point.index),
				}),
			);
			return;
		}
		...
		const timeStep =
			(this.snapMode === "off" || !this.grid
				? this.duration * 0.01
				: this.grid) * factor;
		const valueStep = (this.step || (this.max - this.min) * 0.01) * factor;
		const points = moveEnvelopePoint(
			this._points,
			point.index,
			{
				time: pointValue.time + direction[0] * timeStep,
				value: snapEnvelopeValue(
					pointValue.value + direction[1] * valueStep,
					this.min,
					this.max,
					this.step,
				),
	...
	dispatchSelection() {
		this.dispatchEvent(
			eventOf(
				"envelope-selection",
				this.selection ? { ...this.selection } : { start: null, end: null },
			),
		);
	}
	...
	duplicateSelection() {
		...
		let source = splitEnvelopeAtTime(
			this._points,
			start,
			this.min,
			this.max,
		...
		for (const point of selected) {
			...
			const existing = tagged.find(
				(entry) =>
					entry.point.time === duplicate.time &&
		...
		this.selectionPointIndexes = tagged.flatMap(
			({ selected: isSelected }, index) => (isSelected ? [index] : []),
		);
		this.dispatchSelection();
		this.dispatchEvent(eventOf("envelope-change", { points }));
