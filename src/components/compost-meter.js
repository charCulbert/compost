...
export class CompostMeter extends HTMLElement {
	...
	setState(state = {}) {
		const channels =
			state.channels === undefined ? this.state.channels : state.channels;
		this.state = {
			...this.state,
			...state,
	...
	position(value) {
		return valueToNormalisedPosition(
			this.level(value) ?? this.min,
			this.scaleOptions(),
		);
	}
	...
	render() {
		...
		for (const channel of channels) {
			...
			const overValue = this.level(
				channel.over === true ? channel.primary : channel.over,
			);
			...
			peak.style.bottom =
				peakValue === null ? "0%" : `${this.position(peakValue) * 100}%`;
	...
	bar(name, value) {
		...
		element.style.height =
			value === null ? "0%" : `${this.position(value) * 100}%`;
		return element;
	}
	...
	setLaneAccessibility(lane, channel, label, isMultichannel) {
		...
		lane.setAttribute(
			"aria-label",
			[channel.label, label].filter(Boolean).join(" "),
		);
	...
	scaleOptions() {
		return {
			...
			shape: this.hasAttribute("shape")
				? this.numberAttribute("shape", null)
				: null,
		};
	}
}
