...
export class CompostNumberBox extends HTMLElement {
	...
	readAttributes() {
		...
		this.shape = this.hasAttribute("shape")
			? readNumberAttribute(this, "shape", this.shape ?? 1)
			: null;
	...
	dragScaleFor(event) {
		...
		if (!(width > 0) || !Number.isFinite(event?.clientX))
			return SPLIT_DRAG_SCALES.middle;

		const position = clamp(
			(event.clientX - Number(rect?.left || 0)) / width,
			0,
			1,
		);
		const zone =
			position < 1 / 3 ? "left" : position > 2 / 3 ? "right" : "middle";
		const scale = readNumberAttribute(
			this,
			`drag-step-${zone}`,
			SPLIT_DRAG_SCALES[zone],
		);
		return scale > 0 ? scale : SPLIT_DRAG_SCALES[zone];
	}

	moveDrag(event) {
		if (!this.drag || this.drag.pointerId !== event.pointerId || this.disabled)
			return;
	...
	applyDragDistance(distance, event) {
		...
		const fineScale =
			this.drag.fine || event?.shiftKey
				? this.attributes == null
					? FINE_DRAG_SCALE
					: readNumberAttribute(this, "fine-drag-scale", FINE_DRAG_SCALE)
				: 1;
		const scale = (this.drag.zoneScale || 1) * fineScale;
		const delta = (distance / 180) * scale;
		this.setValue(
			moveValueByNormalisedDelta(this.drag.value, delta, this.scaleOptions()),
			true,
			"control",
		);
	}
	...
	editableValueText() {
		...
		return formatNumber(this._value, this.step, this.displayFractionDigits);
	}

	beginEdit(
		initialValue = this.editableValueText(),
		selectValue = false,
	...
	largeKeyboardStep() {
		...
		return Math.max(
			...
			range / 100 || this.keyboardStep() * 10,
		);
	}
	...
	refresh() {
		...
		if (
			...
			this.box.setAttribute(
				"aria-valuetext",
				this.empty ? this.placeholder || "empty" : valueText,
			);
		}
