...
export class ScopeVisualizer extends HTMLElement {
  ...
  setSamples(samples, { copy = false } = {}) {
    if (this.isTypedSampleArray(samples)) {
      ...
      throw new TypeError('compost-scope.setSamples requires one numeric sample array');
    }
  ...
  requestDraw() {
    if (this._raf || !this.isConnected || typeof requestAnimationFrame !== 'function') return;
  ...
  drawMarkerLabels(width, height, midY) {
    ...
    const ratio = window.devicePixelRatio || 1;
    const labelStyle = getComputedStyle(this.root.querySelector('[data-color="label"]'));
