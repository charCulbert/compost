...
export function snapWithOffset(value, origin, step, mode = "grid") {
	...
	return Math.max(
		0,
		Math.abs(value - offset) <= Math.abs(value - absolute) ? offset : absolute,
	);
}
...
export function normaliseNote(note, beats) {
	const start = clamp(
		Number(note.start) || 0,
		0,
		Math.max(0, beats - MIN_DURATION),
	);
	const duration = clamp(
		Number(note.duration) || MIN_DURATION,
		MIN_DURATION,
		beats - start,
	);
	return {
...
export function normaliseNotes(notes, beats) {
	...
	return notes
		.map((note) => normaliseNote(note, beats))
...
function swungGridBeat(value, step, swing) {
	...
	for (let cell = index - 1; cell <= index + 2; cell += 1) {
		candidates.push(
			cell * step + (Math.abs(cell % 2) === 1 ? step * amount * 0.5 : 0),
		);
	}
...
export function quantizedNotes(
	...
	const result = notes.map((note) => {
		...
		const targetDuration = Math.max(
			step,
			Math.round(note.duration / step) * step,
		);
		const duration = lengths
			? note.duration + (targetDuration - note.duration) * amount
			: note.duration;
		return normaliseNote(
			{ ...note, start, duration },
			Number.isFinite(beats) ? beats : start + duration,
		);
	});
...
export function notesInBox(notes, box) {
	...
	return notes.filter(
		(note) =>
			...
			note.note <= highNote &&
			note.start < endBeat &&
			note.start + note.duration > startBeat,
	);
}
...
export function trimmedNotes(
	...
	return notes.map((note) => {
		...
		const minimum = mode === "off" || !(step > 0) ? MIN_DURATION : step;
		const start = clamp(
			snapWithOffset(note.start + deltaBeats, note.start, step, mode),
			0,
			end - minimum,
		);
...
export function resolveOverlaps(notes, activeIds) {
	...
	for (const note of edited) {
		...
		const end = collisions.length
			? Math.min(...collisions.map((other) => other.start))
			: note.start + note.duration;
		if (end - note.start >= MIN_DURATION)
			winners.push({ ...note, duration: end - note.start });
	}
	const winnerById = new Map(winners.map((note) => [note.id, note]));
	return notes.flatMap((note) => {
		if (active.has(note.id))
			return winnerById.has(note.id) ? [winnerById.get(note.id)] : [];
		const collisions = winners.filter((other) => overlaps(note, other));
		...
		return end - note.start >= MIN_DURATION
...
export function duplicatedNotes(
	...
	const shift =
		range || mode === "off" || !(step > 0)
			? raw
			: Math.max(step, Math.ceil(raw / step - 1e-9) * step);
	return notes
			...
			normaliseNote(
				{
					...
					id: newId(),
					start: Math.min(
						Math.max(0, beats - note.duration),
						note.start + shift,
					),
				},
				beats,
