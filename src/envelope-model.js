...
import { normalisedPositionToValue, valueToNormalisedPosition } from './parameter-scale.js';
...
function geometryArgs(min, max, height, scale) {
  if (min && typeof min === 'object') {
    return { range: envelopeRange(min), height: max, scale: typeof height === 'string' ? height : 'linear' };
  }
...
export function envelopeValueToY(value, min, max, height, scale = 'linear') {
  ...
  const fraction = args.scale === 'gain'
    ...
    : (args.range.max === args.range.min ? .5 : (bounded - args.range.min) / (args.range.max - args.range.min));
...
export function preserveEnvelopeEdgePoints(originPoints, movedPoints, index) {
  ...
  if (pointIndex === 0 && Number(moved.time) > Number(origin.time) + MIN_ENVELOPE_TIME) next.push({ ...origin });
  if (pointIndex === source.length - 1 && Number(moved.time) < Number(origin.time) - MIN_ENVELOPE_TIME) next.push({ ...origin });
...
function editOptions(options = {}) {
  ...
  return {
    ...
    step: effectiveEnvelopeStep(Boolean(options.stepped), options.step ?? options.valueStep),
  };
}
...
function valueMovedByY(value, deltaY, options = {}) {
  ...
  const y = envelopeValueToY(value, range, height, scale) + (Number(deltaY) || 0);
  return snapEnvelopeValue(envelopeValueFromY(y, range, height, scale), range, undefined, step || 0);
}
...
export function moveEnvelopeRangeByY(points, start, end, deltaY, options = {}) {
  ...
  const edgeValues = envelopeRangeEdgeValues(source, low, high, { range, height, scale, stepped, step });
  const selected = source
    .filter((point) => Number(point.time) >= low - MIN_ENVELOPE_TIME && Number(point.time) <= high + MIN_ENVELOPE_TIME)
    .map((point) => ({ ...point, value: valueMovedByY(point.value, deltaY, { range, height, scale, stepped, step }) }));
  const edges = [
    { time: low, value: valueMovedByY(edgeValues.start, deltaY, { range, height, scale, stepped, step }) },
    { time: high, value: valueMovedByY(edgeValues.end, deltaY, { range, height, scale, stepped, step }) },
  ];
  const outside = source.filter((point) => Number(point.time) < low - MIN_ENVELOPE_TIME || Number(point.time) > high + MIN_ENVELOPE_TIME);
  const unique = new Map();
  for (const point of [...outside, ...selected, ...edges]) unique.set(Number(point.time).toFixed(9), point);
...
export function thinEnvelopePoints(points, tolerance = 0) {
  const source = (Array.isArray(points) ? points : [])
    .filter((point) => Number.isFinite(Number(point?.time)) && Number.isFinite(Number(point?.value)))
    .map((point) => ({ time: Math.max(0, Number(point.time)), value: Number(point.value) }))
    .sort((a, b) => a.time - b.time);
  ...
  for (let index = 1; index < source.length - 1; index += 1) {
    ...
    const expected = span > MIN_ENVELOPE_TIME
      ? previous.value + (next.value - previous.value) * (current.time - previous.time) / span
      : previous.value;
    if (span <= MIN_ENVELOPE_TIME || Math.abs(current.value - expected) > limit) result.push(current);
  }
...
export function drawEnvelopePoints(originPoints, samples, options = {}) {
  ...
  const valueStep = effectiveEnvelopeStep(Boolean(options.stepped), options.step ?? options.valueStep);
  ...
  const generated = freehand
    ? tolerance > 0 ? thinEnvelopePoints(raw, tolerance) : raw
    : (() => {
      const width = Math.max(MIN_ENVELOPE_TIME, Number(options.gridStep) || 1);
      const cells = new Map();
      for (const point of raw) cells.set(Math.floor(point.time / width), point.value);
      ...
      for (let cell = first; cell <= last; cell += 1) {
        ...
        result.push({ time: start, value: previous }, { time: Math.max(start, end - MIN_ENVELOPE_TIME), value: previous });
      }
      return result;
    })();
  ...
  const outside = (Array.isArray(originPoints) ? originPoints : [])
    .filter((point) => freehand
      ? Number(point.time) < start - MIN_ENVELOPE_TIME || Number(point.time) > end + MIN_ENVELOPE_TIME
      : Number(point.time) <= replacementStart - MIN_ENVELOPE_TIME || Number(point.time) >= replacementEnd)
    .map((point) => ({ ...point }));
  return [...outside, ...generated].sort((a, b) => a.time - b.time);
}

export function flattenEnvelopeRange(points, start, end, value, min = 0, max = 1, step = 0) {
  const range = envelopeRange(min, max);
  const low = Math.min(Number(start) || 0, Number(end) || 0);
  const high = Math.max(Number(start) || 0, Number(end) || 0);
  if (!(high > low + MIN_ENVELOPE_TIME)) return (Array.isArray(points) ? points : []).map((point) => ({ ...point }));
  ...
  const edges = [
    ...
    { time: high, value: snapEnvelopeValue(rightValue, range, undefined, step) },
  ];
  return [...outside, ...edges].sort((a, b) => Number(a.time) - Number(b.time));
}

export function moveEnvelopeRange(points, start, end, delta, min = 0, max = 1, step = 0) {
  const range = envelopeRange(min, max);
  const low = Math.min(Number(start) || 0, Number(end) || 0);
  const high = Math.max(Number(start) || 0, Number(end) || 0);
  if (!(high > low + MIN_ENVELOPE_TIME)) return (Array.isArray(points) ? points : []).map((point) => ({ ...point }));
  const source = (Array.isArray(points) ? points : []).map((point) => ({ ...point }));
  const selected = source.filter((point) => Number(point.time) >= low - MIN_ENVELOPE_TIME && Number(point.time) <= high + MIN_ENVELOPE_TIME);
  const edgeValue = (time) => envelopeValueAtTime(source, time, range);
  const edgePoints = [{ time: low, value: edgeValue(low) }, { time: high, value: edgeValue(high) }];
  const moved = [...selected, ...edgePoints].map((point) => ({
    ...point,
    value: snapEnvelopeValue(Number(point.value) + Number(delta || 0), range, undefined, step),
  }));
  const outside = source.filter((point) => Number(point.time) < low - MIN_ENVELOPE_TIME || Number(point.time) > high + MIN_ENVELOPE_TIME);
  const unique = new Map();
  for (const point of [...outside, ...moved]) unique.set(Number(point.time).toFixed(9), point);
  return [...unique.values()].sort((a, b) => Number(a.time) - Number(b.time));
}
