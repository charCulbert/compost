...
function setMIDIMapState(target, token, enabled) {
	...
	const tokens = new Set(
		String(target.getAttribute?.(MIDI_MAP_STATE_ATTRIBUTE) || "")
			.split(/\s+/u)
			.filter(Boolean),
	);
	...
	if (tokens.size)
		target.setAttribute?.(MIDI_MAP_STATE_ATTRIBUTE, [...tokens].join(" "));
...
function mappableTargetFromNode(node, selector = MAPPABLE_SELECTOR) {
	...
	return node.matches?.(selector) ? node : node.closest?.(selector) || null;
}
...
function isTextEditingTarget(event) {
	...
	const type = String(
		target.type || target.getAttribute?.("type") || "text",
	).toLowerCase();
...
export class MIDILearnUI {
	...
	connect() {
		...
		this.mappings?.addEventListener?.(
			"midi-learn-begin",
			this.handleLearnStart,
		);
		this.mappings?.addEventListener?.(
			"midi-learn-cancel",
			this.handleLearnCancel,
		);
	...
	disconnect() {
		...
		this.mappings?.removeEventListener?.(
			"midi-learn-begin",
			this.handleLearnStart,
		);
		this.mappings?.removeEventListener?.(
	...
	handleAudioStopped() {
		...
		this.announce("MIDI map mode exited because audio stopped or suspended.", {
			priority: "assertive",
		});
	}
	...
	handleFocusOut(event) {
		...
		const nextTarget = mappableTargetFromNode(
			event.relatedTarget,
			this.selector,
		);
	...
	handleKeyDown(event) {
		...
		if (
			...
			const target =
				eventTarget && eventTarget !== this.button
					? eventTarget
	...
	beginSelecting() {
		...
		this.clearTargetHighlight();
		this.announce(
			"MIDI map mode active. Focus a control, then move a MIDI CC.",
			{ priority: "assertive" },
		);
	}
	...
	handleLearnStart(event) {
		...
		const mapping = this.lastTarget
			? this.mappingLabelForTarget(this.lastTarget)
			: "";
		const existing = mapping
			? ` Mapped to ${this.mappingSpeechLabelForTarget(this.lastTarget) || mapping}.`
			: "";
		const prefix = this.announceLearnStartAsEntry
			? "MIDI map mode active. "
			: "";
		const priority = this.announceLearnStartAsEntry ? "assertive" : "polite";
		this.announceLearnStartAsEntry = false;
		this.announce(
			`${prefix}${label}.${existing} Move a CC to ${existing ? "remap" : "map"}.`,
			{ priority },
		);
	}
	...
	handleMap(event) {
		...
		if (
			mappedTarget &&
			this.mappings?.beginLearn?.(mappableTargetParameterID(mappedTarget))
		) {
			this.setModeState("learning");
	...
	handleLearnCancel() {
		...
		if (this.isExiting || performance.now() < this.ignoreLearnCancelUntil)
			return;
	...
	updateIdleLabel() {
		...
		this.button?.setAttribute(
			"aria-description",
			"Activate to enter MIDI map mode.",
		);
		this.setState("idle");
		if (this.state === "idle") {
			this.stopPulse();
	...
	setState(state) {
		...
		if (state === "idle") {
			...
			this.button.setAttribute(
				"aria-description",
				"MIDI map mode active. Escape exits.",
			);
		}
	}
	...
	syncLearnVisuals(state, target) {
		...
		this.visualFrame = requestAnimationFrame(() => {
			...
			setMIDIMapState(this.highlightedTarget, "active", true);
			this.showMappingLabels();
			this.describeMappingTarget(
				this.highlightedTarget,
				this.mappingLabelForTarget(this.highlightedTarget),
			);
			this.setPressed(true);
			this.setState(state);
			this.startPulse();
	...
	describeMappingTarget(target, mappingLabel = "") {
		...
		if (!this.originalTargetDescriptions.has(target)) {
			this.originalTargetDescriptions.set(target, {
				...
				describedByID:
					target.shadowRoot?.querySelector?.("[data-midi-map-description]")
						?.id || "",
	...
	restoreMappingTargetDescription(target) {
		...
		for (const {
			...
			ariaDescription,
			ariaDescribedBy,
		} of original.focusableDescriptions || []) {
		...
		const descriptionNode = target.shadowRoot?.querySelector?.(
			"[data-midi-map-description]",
		);
	...
	focusableNodesForTarget(target) {
		...
		return [
			...target.shadowRoot.querySelectorAll(
				"button, input, select, textarea, [role], [tabindex]",
			),
	...
	mappingControlKind(target, focusableNodes) {
		...
		if (
			...
			focusableNodes.some(
				(node) =>
					node.type === "radio" || node.getAttribute?.("role") === "radio",
			)
		) {
			return "choice";
		}
		...
		if (
			...
			focusableNodes.some((node) => node.tagName === "BUTTON")
		) {
			return "button";
		}
		...
		if (
			focusableNodes.some(
				(node) => node.getAttribute?.("role") === "spinbutton",
			)
		) {
			return "number";
		}
		...
		if (
			focusableNodes.some((node) => node.getAttribute?.("role") === "slider")
		) {
			return "continuous";
		}
	...
	mappingActionText(kind) {
		if (kind === "choice")
			return "Mapped CC values will choose between the options in this group.";
		if (kind === "button")
			return "Mapped CC values will trigger or toggle this control.";
		if (kind === "number")
			return "Mapped CC values will set this numeric value.";
