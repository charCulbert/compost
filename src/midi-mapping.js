...
export function readMIDIMessage(event) {
	return (
		event.detail?.message || event.detail?.data || event.detail || event.data
	);
}
...
export function normaliseMIDIMapping(mapping = {}) {
	...
	const mappedChannel =
		rawChannel === null || rawChannel === undefined ? null : Number(rawChannel);
...
export function midiMappingFromMessage(
	message,
	{ learnChannel = true, channel = null } = {},
) {
...
export function midiMappingMatchesMessage(mapping, message) {
	...
	return (
		normalised.cc === cc &&
		(normalised.channel === null || normalised.channel === channel)
	);
}
