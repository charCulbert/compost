...
function normaliseMapping(mapping, parameterProvider) {
	...
	if (rawMin > rawMax || rawMin < definition.min || rawMax > definition.max)
		return null;

	const min = rawMin;
	const max = rawMax;
	const rawMid =
		mapping.mid ?? mapping.midiMid ?? mapping.mappingMid ?? definition.mid;
	const mid =
		rawMid === null || rawMid === undefined || rawMid === ""
			? null
			: Number(rawMid);
	if (
		mid !== null &&
	...
	const rawShape =
		...
		definition.shape;
	const shape =
		rawShape === null || rawShape === undefined || rawShape === ""
			? null
			: Number(rawShape);
...
function valueForMapping(mapping, ccValue) {
	...
	if (mapping.values?.length) {
		const values = mapping.values.filter(
			(candidate) =>
				candidate >= mapping.min - 1e-9 && candidate <= mapping.max + 1e-9,
		);
		if (values.length && values.length <= 128) {
			const ordered =
				mapping.min <= mapping.max ? values : [...values].reverse();
			return ordered[
				Math.min(ordered.length - 1, Math.floor((value * ordered.length) / 128))
			];
		}
	}
	...
	if (mapping.step > 0) {
		...
		const count =
			Math.abs(intervalEstimate - intervalCount) <= 1e-9
				? intervalCount + 1
				: 0;
	...
	if (mapping.step > 0) {
		const snapped =
			mapping.min +
			Math.round((mapped - mapping.min) / mapping.step) * mapping.step;
...
export class MIDIMappings extends EventTarget {
	...
	beginLearn(parameterID, { learnChannel = this.learnChannel } = {}) {
		...
		this.dispatchEvent(
			new CustomEvent("midi-learn-begin", {
	...
	cancelLearn() {
		...
		this.learning = null;
		this.dispatchEvent(
			new CustomEvent("midi-learn-cancel", { detail: { parameterID } }),
		);
		return true;
	}
	...
	requestSet(mapping) {
		...
		this.dispatchEvent(
			new CustomEvent("midi-mapping-request", {
	...
	requestClear(parameterID) {
		...
		this.dispatchEvent(
			new CustomEvent("midi-unmapping-request", {
				detail: { parameterID: id },
			}),
		);
		return true;
	}
	...
	applyMapping(mapping) {
		...
		this.dispatchEvent(
			new CustomEvent("midi-map", { detail: mappingDetail(normalised) }),
		);
		return true;
	}
	...
	applyMappings(mappingList = []) {
		...
		const normalised = mappingList.map((mapping) =>
			normaliseMapping(mapping, this.parameterProvider),
		);
		if (normalised.some((mapping) => !mapping)) return false;
		const parameterIDs = new Set(
			normalised.map((mapping) => mapping.parameterID),
		);
		if (parameterIDs.size !== normalised.length) return false;

		const next = new Map(
			normalised.map((mapping) => [mapping.parameterID, mapping]),
		);
		...
		for (const [parameterID, mapping] of previous) {
			if (!next.has(parameterID)) {
				this.dispatchEvent(
					new CustomEvent("midi-unmap", { detail: mappingDetail(mapping) }),
				);
			}
		}
		for (const mapping of next.values()) {
			this.dispatchEvent(
				new CustomEvent("midi-map", { detail: mappingDetail(mapping) }),
			);
		}
		return true;
	}
	...
	handleMIDIMessage(event) {
		const message =
			event?.detail?.message ?? event?.detail?.data ?? event?.data ?? event;
		...
		if (this.learning !== null) {
			...
			if (requested) {
				this.dispatchEvent(
					new CustomEvent("midi-learn-captured", {
			...
			if (!requested) {
				this.dispatchEvent(
					new CustomEvent("midi-learn-cancel", {
		...
		const matching = [...this.mappingState.values()].filter((candidate) =>
			midiMappingMatchesMessage(candidate, message),
		);
