...
test("custom visuals expose independent keyboard values and shared parameter intent", async ({
	...
	const drive = page.locator("#drive-control");
	const repeats = page.locator("#repeats-control");
	await expect(page.locator("#custom-canvas")).toHaveCSS(
		"touch-action",
		"none",
	);
	await expect(drive).toHaveAccessibleName("Drive");
	await expect(drive).toHaveAttribute("aria-valuemin", "0");
	await expect(drive).toHaveAttribute("aria-valuemax", "1");
	await expect(drive).toHaveAttribute("aria-valuenow", "0.25");
	await expect(repeats).toHaveAccessibleName("Number of repeats");
	await expect(repeats).toHaveAttribute("aria-valuenow", "4");
	await drive.focus();
	await page.keyboard.press("ArrowUp");
	await expect(drive).toHaveAttribute("aria-valuenow", "0.26");
	await expect(page.locator("compost-knob")).toHaveJSProperty("value", 0.26);
	await expect(repeats).toHaveAttribute("aria-valuenow", "4");
	await page.keyboard.press("Tab");
	await expect(repeats).toBeFocused();
	await page.keyboard.press("ArrowUp");
	await expect(repeats).toHaveAttribute("aria-valuenow", "5");
	await expect(drive).toHaveAttribute("aria-valuenow", "0.26");
	...
	await expect(repeats).toHaveAttribute("aria-valuenow", "16");
	await page.keyboard.press("Home");
	await expect(repeats).toHaveAttribute("aria-valuenow", "1");
});

test("host values stay silent across custom and built-in controls", async ({
	page,
}) => {
	...
	await page.locator("#drive-control").focus();
...
test("custom canvas drag cancels once and restores both synchronized views", async ({
	...
	await expect(page.locator("#drive-control")).toBeFocused();
	expect(
		Number(await page.locator("#drive-control").getAttribute("aria-valuenow")),
...
test("number-of-repeats end bracket uses absolute positioning", async ({
	...
	const canvas = page.locator("#custom-canvas");
	const rect = await canvas.boundingBox();
	const trailLeft = rect.x + rect.width / 2;
	const trailWidth = rect.width / 2;
	const initialX = trailLeft + trailWidth * (3 / 15);
	const targetX = trailLeft + trailWidth * (11 / 15);
	const y = rect.y + rect.height / 2;
	await page.mouse.move(initialX, y);
	await expect(page.locator("#repeats-control")).toHaveCSS(
		"cursor",
		"ew-resize",
	);
	await page.mouse.down();
	...
	await page.mouse.up();
	await expect(page.locator("#repeats-control")).toHaveAttribute(
		"aria-valuenow",
		"12",
	);
	await expect(page.locator("#drive-control")).toHaveAttribute(
...
test("relative canvas movement rebases after a host update and Shift change", async ({
	...
	const beforeFine = Number(
		await page.locator("#drive-control").getAttribute("aria-valuenow"),
	);
