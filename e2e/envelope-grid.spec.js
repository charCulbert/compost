...
test("envelope time and value grids are opt-in and logically derived", async ({
	...
	const state = await page.evaluate(async () => {
		...
		const bipolarValues = [...grid.querySelectorAll(".value-grid-line")].map(
			(line) => Number(line.dataset.value),
		);
		editor.setAttribute("min", "0");
		editor.setAttribute("max", "1");
		editor.setAttribute("stepped", "");
		editor.setAttribute("step", ".25");
		const steppedValues = [...grid.querySelectorAll(".value-grid-line")].map(
			(line) => Number(line.dataset.value),
		);
	...
	expect(state.initial.time).toBeCloseTo(0.037, 3);
	expect(state.bipolarValues).toEqual([0]);
	expect(state.steppedValues).toEqual([0.25, 0.5, 0.75]);
	expect(state.configured.grid).toBe(0.01);
	expect(state.configured.snap).toBe("grid");
	expect(state.configured.backgroundSize).toBe("8% 100%");
	expect(state.configured.time).toBe(0.04);
	expect(state.hidden.background).toBe("none");
	expect(state.hidden.valueLines).toBe(0);
	expect(state.hidden.time).toBe(0.04);
});
