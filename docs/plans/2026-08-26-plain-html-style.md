...
Open `http://127.0.0.1:8931/review.html?el=<element>`. It renders the real
element from this branch in three page contexts: unstyled, `color-scheme:
dark`, and a branded page (serif, 18px, `--compost-accent`). Check:
...
- Type declarations (`src/components/*.d.ts`) live on local `main`, not on
  this branch: rebase before merging and add `relative` to the slider types.
