...
if (!globalThis.addEventListener || !globalThis.removeEventListener || !globalThis.dispatchEvent) {
...
export class FakeControl extends EventTarget {
  ...
  matchesSingle(selector) {
    ...
    if (selector === '[midi-map-target]') return this.hasAttribute('midi-map-target');
    if (selector === 'compost-knob' || selector === 'compost-knob[name]') {
      return ['COMPOST-KNOB', 'SYNTH-KNOB'].includes(this.tagName)
        && (selector === 'compost-knob' || this.hasAttribute('name'));
    }
    if (selector === 'compost-slider' || selector === 'compost-slider[name]') {
      return ['COMPOST-SLIDER', 'PARAMETER-SLIDER'].includes(this.tagName)
