...
test('the Signal Generator uses the current one-channel scope contract', () => {
  ...
  assert.match(html, /<compost-select[^>]+parameter-id="scopeWindow"/u);
  assert.match(html, /<compost-number-box[^>]+parameter-id="scopeSamples"/u);
  assert.match(html, /<compost-number-box[^>]+parameter-id="scopePeriods"/u);
  ...
  assert.match(main, /scope\.setSamples\(samples\)/u);
  assert.match(main, /displayValues\.scopePeriods \* \(audio\?\.context\.sampleRate/u);
