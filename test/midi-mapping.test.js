...
test('midiMappingFromMessage learns CC and channel from a CC message', () => {
  ...
  assert.deepEqual(midiMappingFromMessage([0xb2, 74, 100], { learnChannel: false }), {
    cc: 74,
    midiCC: 74,
    channel: null,
    midiChannel: null,
...
test('midiMappingMatchesMessage honors fixed channel and wildcard channel', () => {
  assert.equal(midiMappingMatchesMessage({ cc: 10, channel: 2 }, [0xb1, 10, 64]), true);
  assert.equal(midiMappingMatchesMessage({ cc: 10, channel: 2 }, [0xb2, 10, 64]), false);
  assert.equal(midiMappingMatchesMessage({ cc: 10, channel: null }, [0xbf, 10, 64]), true);
});
...
test('mapMIDIValueToRange maps 7-bit MIDI to the full parameter range', () => {
  ...
  assert.ok(Math.abs(mapMIDIValueToRange(64, { min: 0, max: 10 }) - 5.03937) < 0.0001);
});

test('mapMIDIValueToRange uses linear mid as a two-segment center', () => {
  assert.equal(mapMIDIValueToRange(63.5, {
    min: 0,
    max: 100,
    mid: 10,
  }), 10);
...
test('mapMIDIValueToRange supports log curves', () => {
  assert.ok(Math.abs(mapMIDIValueToRange(63.5, {
    min: 20,
    max: 20000,
    mid: 1000,
    curve: 'log',
