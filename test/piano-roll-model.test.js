...
test("free editing preserves sub-sample-scale beat values", () => {
	...
	assert.equal(
		normaliseNotes([note({ start: precise, duration: precise })], 1)[0].start,
		precise,
	);
});
...
test("resizing grows from the right edge and is capped by the clip", () => {
	...
	assert.equal(
		resizedNotes(notes, ["a"], -99, 4, 0.25)[0].duration,
		0.25,
...
test("resolveOverlaps removes covered starts only on the same pitch and channel", () => {
	...
	const otherChannel = note({
		id: "channel",
		start: 1.5,
		duration: 1,
		channel: 3,
	});
	const otherPitch = note({
		id: "pitch",
		note: 61,
		start: 1.5,
		duration: 1,
	...
	assert.deepEqual(
		resolveOverlaps(
			[active, covered, touching, otherChannel, otherPitch],
			["active"],
		),
...
test("quantize can be limited to a selection", () => {
	const notes = [
		note({ id: "a", start: 0.31 }),
		note({ id: "b", start: 1.31 }),
	];
...
test("quantize supports partial strength and swung starts", () => {
	...
	const swung = quantizedNotes([note({ start: 0.36 })], 0.25, {
		swing: 1,
		beats: 4,
	});
