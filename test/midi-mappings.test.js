...
test('mapping request stays unconfirmed until applyMapping', () => {
  ...
  let requested = null;
  mappings.addEventListener('midi-mapping-request', (event) => { requested = event.detail; });
  assert.equal(mappings.requestSet({ parameterID: 'gain', cc: 7, channel: 1 }), true);
...
test('learning completes with a mapping request without cancelling map mode', () => {
  ...
  mappings.addEventListener('midi-mapping-request', ({ detail }) => events.push(detail.cc));
...
test('external backend mode learns and confirms mappings without local MIDI execution', () => {
  ...
  assert.equal(mappings.applyMapping({ parameterID: backendLearning, cc: 74, channel: 4 }), true);
...
test('local MIDI emits intent without mutating ParameterController', () => {
  ...
  let intent = null;
  mappings.addEventListener('midi-parameter', (event) => { intent = event.detail; });
...
test('one MIDI address fans out to multiple parameter targets', () => {
  const parameters = createParameterController({
    ...
    definitions: [
      { parameterID: 'gain', kind: 'continuous', min: 0, max: 1, defaultValue: 0, step: 0 },
  ...
  const targets = [];
  mappings.addEventListener('midi-parameter', ({ detail }) => targets.push(detail.parameterID));
  assert.equal(mappings.handleMIDIMessage([0xb0, 7, 127]), true);
...
test('mapping snapshots expose ranges and cannot mutate controller state', () => {
  const control = new FakeControl({ 'parameter-id': 'mode', min: 0, max: 3, value: 0 });
  const parameters = createParameterController({
...
test('mapping ranges and curve metadata stay inside parameter legality', () => {
  const { mappings } = setup();
  assert.equal(mappings.requestSet({ parameterID: 'gain', cc: 3, min: -0.1, max: 1 }), false);
  assert.equal(mappings.requestSet({ parameterID: 'gain', cc: 3, min: 0, max: 1.1 }), false);
  assert.equal(mappings.requestSet({ parameterID: 'gain', cc: 3, min: 0.8, max: 0.2 }), false);
  assert.equal(mappings.requestSet({ parameterID: 'gain', cc: 3, min: 0, max: 1, mid: 2 }), false);
  assert.equal(mappings.requestSet({ parameterID: 'gain', cc: 3, min: 0, max: 1, shape: 0 }), false);
...
test('discrete and stepped mappings use stable MIDI buckets', () => {
  ...
  const steppedValues = [];
  mappings.addEventListener('midi-parameter', ({ detail }) => steppedValues.push(detail.value));
...
test('applyMappings validates the full snapshot before replacing mappings', () => {
  ...
  assert.equal(mappings.applyMappings([
    { parameterID: 'gain', cc: 8 },
    { parameterID: 'gain', cc: 9 },
  ]), false);
  assert.equal(mappings.get('gain').cc, 7);
  assert.equal(mappings.applyMappings([{ parameterID: 'missing', cc: 10 }]), false);
  assert.equal(mappings.get('gain').cc, 7);
