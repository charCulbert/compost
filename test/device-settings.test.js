...
test('normaliseDeviceSelectorSnapshot normalises canonical device selector snapshots', () => {
  const snapshot = normaliseDeviceSelectorSnapshot({
    ...
    midi: {
      inputDevices: [{ id: 'keyboard', name: 'Keyboard' }, { id: 'pads', name: 'Pads' }],
      outputDevices: [{ id: 'synth', name: 'Synth' }, { id: 'clock', name: 'Clock' }],
  ...
  assert.deepEqual(snapshot.midi.inputDevices.map((device) => device.id), ['keyboard', 'pads']);
  assert.deepEqual(snapshot.midi.outputDevices.map((device) => device.id), ['synth', 'clock']);
