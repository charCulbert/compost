...
function setup() {
	...
	const secondTarget = new FakeControl({
		"parameter-id": "tone",
		label: "Tone",
		min: 0,
		max: 1,
	...
	return {
		...
		secondTarget,
		mappings,
		ui: new MIDILearnUI({ mappings, root, button, status }),
		status,
	};
}

function hasMIDIMapState(target, state) {
	return String(target.getAttribute("midi-map-state") || "")
		.split(/\s+/u)
		.includes(state);
}
...
test("MIDILearnUI focus starts learning and announces confirmed mappings", async () => {
	...
	assert.match(
		status.textContent,
		/Gain\. Mapped to MIDI channel 2, CC 74\. Move a CC to remap/u,
	);
	...
	assert.match(
		target.getAttribute("aria-description"),
		/Mapped to MIDI channel 2, CC 74/u,
	);
