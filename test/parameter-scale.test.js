...
test("gain curve remains invertible beyond the canonical range", () => {
	...
	for (const value of [-120, -90, 0, 12, 24]) {
		assertClose(
			normalisedPositionToValue(valueToNormalisedPosition(value, scale), scale),
			value,
		);
	}
});
...
test("gain curve exposes only its built-in slope changes inside a range", () => {
	assert.deepEqual(
		parameterScaleBreakpoints({ min: -24, max: 6, curve: "gain" }),
		[-12, -6, 0],
	);
...
test("curve and shape do not accept aliases", () => {
	for (const alias of [{ taper: "log" }, { scale: "log" }]) {
		assert.equal(
			normalisedPositionToValue(0.5, { min: 0, max: 10, ...alias }),
			5,
		);
	}
...
test("scale description is stable for previews", () => {
	...
	assert.equal(
		describeParameterScale({ min: -90, max: 12, curve: "gain" }),
		"gain curve",
	);
});
