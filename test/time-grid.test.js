...
test("adaptive grids share one readable zoom rule", () => {
	assert.equal(adaptiveGridStep(24, 4), 0.5);
	assert.equal(adaptiveGridStep(48, 4), 0.25);
	...
	assert.equal(gridStepForView(4, "1/32", 24, true, 8), 0.0625);
	assert.equal(gridStepForView(4, "1/32", 24, true, 0.125), 4);
	assert.equal(gridStepForView(4, "1/32", 24, true, 20), 0.0625);
	assert.equal(gridStepForView(4, "1/32", 24, true, 0.01), 4);
...
test("snapping keeps an origin offset when that is nearer than the grid", () => {
	...
	assert.ok(
		Math.abs(snapTime(1.29, { step: 0.25, origin: 0.31 }) - 1.31) < 1e-12,
	);
	// moved by ~1.2 the grid line is nearer
	assert.ok(
		Math.abs(snapTime(1.49, { step: 0.25, origin: 0.31 }) - 1.5) < 1e-12,
	);
