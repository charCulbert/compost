...
test('external definitions override control semantics but preserve presentation', () => {
  const control = new FakeControl({ 'parameter-id': 'frequency', min: 1, max: 2, curve: 'log', mid: 1000, value: 440 });
  const parameters = createParameterController({ root: new FakeRoot([control]), definitions: [{ parameterID: 'frequency', kind: 'continuous', min: 20, max: 20000, defaultValue: 440, step: 0, unit: 'Hz' }] });
...
test('controller updates generic parameter-id elements without setValue', () => {
  const control = new FakeControl({ 'parameter-id': 'gain', min: 0, max: 1, value: 0.2 });
...
test('controller re-emits lifecycle and synchronizes sibling user edits', () => {
  ...
  for (const type of ['parameter-begin', 'parameter-edit', 'parameter-end']) {
    parameters.addEventListener(type, ({ detail }) => events.push([type, detail.cancelled]));
  }
  parameters.handleEvent({ type: 'parameter-begin', detail: { parameterID: 'gain', value: 0.2 } });
  parameters.handleEvent({ type: 'parameter-edit', target: first, detail: { parameterID: 'gain', value: 0.7 } });
  parameters.handleEvent({ type: 'parameter-end', detail: { parameterID: 'gain', value: 0.7 } });
...
test('local definitions use the current value when no reset is declared', () => {
  const control = new FakeControl({ 'parameter-id': 'gain', min: 0, max: 1, value: 0.25 });
  const parameters = createParameterController({ root: new FakeRoot([control]) });
...
test('only explicit non-empty parameter IDs register', () => {
  const named = new FakeControl({ name: 'frequency', min: 20, max: 20000, value: 440 });
  const empty = new FakeControl({ 'parameter-id': '', min: 0, max: 1, value: 0 });
  const parameters = createParameterController({ root: new FakeRoot([named, empty]) });
...
test('local controls with conflicting semantics fail clearly', () => {
  const first = new FakeControl({ 'parameter-id': 'gain', min: 0, max: 1, value: 0 });
  const second = new FakeControl({ 'parameter-id': 'gain', min: 0, max: 2, value: 0 });
  assert.throws(
...
test('external definitions mirror defaults without rendered controls', () => {
  ...
  parameters.setDefinitions([{ parameterID: 'gain', min: 0, max: 0.5, defaultValue: 0.1 }]);
  assert.equal(parameters.value('gain'), 0.1);
});

test('definition rescans update semantics without changing presentation', () => {
  const control = new FakeControl({ 'parameter-id': 'gain', min: 0, max: 1, curve: 'log', value: 0.5 });
  const parameters = createParameterController({
    root: new FakeRoot([control]),
    definitions: [{ parameterID: 'gain', min: 0, max: 1, defaultValue: 0.5 }],
  });
  parameters.setDefinitions([{ parameterID: 'gain', min: -1, max: 1, defaultValue: 0 }]);
...
test('applyValues rejects an invalid batch without partial updates', () => {
  const control = new FakeControl({ 'parameter-id': 'gain', min: 0, max: 1, value: 0.2 });
  const parameters = createParameterController({ root: new FakeRoot([control]) });
  assert.equal(parameters.applyValues([
...
test('disconnect removes root listeners cleanly', () => {
  ...
  let edits = 0;
  parameters.addEventListener('parameter-edit', () => edits += 1);
  root.dispatchEvent(new CustomEvent('parameter-edit', {
    bubbles: true,
