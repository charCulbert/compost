...
const parameters = createParameterController({
  root: document,
  definitions: [{ parameterID: 'frequency', kind: 'continuous', name: 'Frequency', min: 20, max: 20000, defaultValue: 440, step: 0, unit: 'Hz' }],
});
...
parameters.addEventListener('parameter-edit', (event) => setBackendValue(event.detail.parameterID, event.detail.value));
document.querySelector('#set-880').addEventListener('click', () => setBackendValue('frequency', 880));
