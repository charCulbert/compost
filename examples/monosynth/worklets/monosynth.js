...
class CompostMonoSynth extends AudioWorkletProcessor {
	...
	handleMessage(data) {
		...
		if (data?.type === "sequence") {
			...
			this.loopEnd = Math.max(
				this.loopStart + 0.001,
				Number(data.loopEnd) || 4,
			);
		...
		if (data?.type === "noteOff")
			this.liveNotes.delete(`${data.source ?? "live"}:${data.note}`);
	}
	...
	updateVoice(attack, release) {
		...
		if (!target && this.playing) {
			for (const note of this.notes) {
				if (
					note.start <= this.beat &&
					this.beat < note.start + note.duration &&
	...
	publishCapture() {
		...
		this.port.postMessage(
			{ type: "scope-samples", samples, outputSamples, beat: this.beat },
			[samples.buffer, outputSamples.buffer],
		);
	}
}
...
function pitchEnvelopeValue(points, time) {
	...
	for (let index = 1; index < points.length; index += 1) {
		...
		const position = envelopeCurvePosition(
